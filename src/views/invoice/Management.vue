...
<template>
  <div>
    ...
    <el-upload class="upload-demo" action="http://127.0.0.1:8000" :on-success="handleSuccess" :before-upload="beforeUpload">
    <el-button type="primary" icon="el-icon-upload2">点击上传ZIP文件</el-button>
    </el-upload>
...
export default {
  ...
  methods: {
    ...
    parseUploadedFile(file) {
      ...
      // 发送请求给后端进行解析和入库
      axios.post('http://127.0.0.1:8000/upload-invoice', formData, { headers })
        .then(response => {
          if (response.data.success) {
            this.$message.success('文件解析成功！');
            // 刷新界面或执行其他操作，显示解析结果等
          } else {
            this.$message.error(response.data.message || '文件解析失败！');
          }
        })
        .catch(error => {
          this.$message.error('文件解析失败，请稍后再试！');
        });
    },
